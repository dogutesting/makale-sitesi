...
const MAX_TIMEOUT = 12 * 60 * 60 * 1000; // 12 saat
...
export default function rateLimit() {
  return async (req, res, next) => {
    ...
    if (!limits) {
      ...
      limits = {
        remaining: 10, // İzin verilen istek sayısı
        resetTime: Date.now() + DEFAULT_TIMEOUT, // Kota sıfırlama zamanı
      ...
      next();
        ...
        // Limits varsa
        const lastRequestTime = limits.lastRequestTime || 0;
        const elapsedTime = Date.now() - lastRequestTime;
        if (elapsedTime < 60000) { // 60 saniyede bir 10 kotası olacak
