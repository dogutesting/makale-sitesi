...
const allCategories = ["dizi", "film", "motosiklet"];
const items = [];
...
export default function Ens_all({kategori, jsonContentArray, sayfaUrl, currentPageOperations}) {
  ...
  useEffect(() => {
    console.log("ens_all render edildi.");
  }, []);

  const DynamicComponent = dynamic(() => {
    return import(`../kunyeler/${kategori}_kunye`)
  }, {ssr: true})
        ...
        items.push(
          <section key={index}>
            <En
                ...
                <DynamicComponent
                 ozellikler={item.ozellikler} parseHtml={cHtmlToJsx}/>
  ...
  useEffect(() => {
    DynamicComponent.isReady
  }, [])
  return (
    <>
      {/* <DynamicComponent ozellikler={{"doğukan": "sayın"}} parseHtml={cHtmlToJsx}/> */}
      {items}
    </>
  )
}
